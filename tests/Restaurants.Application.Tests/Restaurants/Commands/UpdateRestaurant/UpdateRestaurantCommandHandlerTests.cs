...
public class UpdateRestaurantCommandHandlerTests
{
    ...
    public async Task Handle_WithValidRequest_ShouldUpdateRestaurants()
    {
        ...
        _restaurantsRepositoryMock.Setup(r => r.GetByIdAsync(restaurantId))
            .ReturnsAsync(restaraunt);

        _restaurantAuthorizationServiceMock.Setup(m => m.Authorize(restaraunt, ResourceOperation.Update))
            .Returns(true);
        ...
        _mapperMock.Verify(m => m.Map(command, restaraunt), Times.Once);
    }
